...
- **Demo-profile seed data** (`src/main/java/me/cleanbrain/relayhub/demo/DemoDataSeeder.java`, `application-demo.yml`, 2026-09-10): a `CommandLineRunner` gated on the `demo` Spring profile idempotently registers 2 Sources (`demo-erp`, `demo-crm`), their `customer-created` SourceEvent, 2 Targets (`demo-warehouse` — always succeeds, `demo-billing` — will randomly fail), and all 4 Subscriptions between them, on every startup. Off by default; reuses `SourceService`/`SourceEventService`/`TargetService`/`SubscriptionService` (same validation path as the HTTP API), not a direct-repository shortcut. Verified live against real Postgres/Kafka: first run seeded all 10 entities with no errors; a full app restart with the same profile re-ran the seeder with zero "Seeded ..." lines (everything already existed, no exceptions) and a follow-up ingress call confirmed `deliveryCount: 2` (unchanged) — no duplicate Subscriptions created. Exists to give the still-to-be-built developer-facing observability dashboard something to show without manual registration first, and as the target Source/Target keys the separate `relayhub-demo-systems` simulator repo (not yet created) will call/be called by.

## In progress

- Designing and scaffolding the separate `relayhub-demo-systems` repository: a simulator exposing `/targets/warehouse` (always succeeds) and `/targets/billing` (randomly fails, driven by its own internal logic — deliberately not RelayHub's or a shell script's responsibility), plus a background scheduler that continuously and automatically POSTs synthetic `customer-created` events to RelayHub's `/ingress/v1/demo-erp/customer-created` and `/ingress/v1/demo-crm/customer-created` without any manual action, so the eventual developer site has continuous real traffic (including real DLQ activity) to show as Observability. Protocol is JSON/HTTP only, fixed by explicit decision (2026-09-10) — no protocol-extensibility field for now.
...
1. Build `relayhub-demo-systems` (see In progress) and wire its Targets' `baseUrl` into the seeded demo Targets (currently `http://localhost:9500`, from `relayhub.demo.simulator-base-url`).
...
3. Add `V2__...` migrations as real schema changes arise — `ddl-auto` will no longer silently apply them; each needs a deliberate SQL file and a `validate` check.
4. CI currently only runs tests on push/PR (`.github/workflows/ci.yml`) — no build/publish/deploy job exists, deliberately, since there is no deployment target yet (item 2). Add one once item 2 is decided.
5. `management.tracing.sampling.probability: 1.0` traces every request — fine at personal-project scale, would need dialing down before any real traffic volume (see `specs/004-observability/spec.md`).
