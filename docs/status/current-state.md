...
- Designing and scaffolding the separate `relayhub-demo-systems` repository (Node.js/TypeScript, per maintainer choice 2026-09-10): a simulator exposing `/targets/airport-display` (always succeeds) and `/targets/travelapp-vendor` (randomly fails/times out, driven by its own internal logic — deliberately not RelayHub's or a shell script's responsibility), plus a background scheduler that continuously and automatically sends synthetic `flight-created`/`flight-status-updated` events to RelayHub's `/ingress/v1/demo-flightstatus/...` URLs without any manual action, so the eventual developer site has continuous real traffic (including real DLQ activity) to show as Observability. Protocol is JSON/HTTP only, fixed by explicit decision (2026-09-10) — no protocol-extensibility field for now.
