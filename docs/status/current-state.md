...
1. Add a `Dockerfile` to `relayhub-demo-systems` and wire it into `relayhub-java`'s `docker-compose.yml` as a service, so `docker compose up -d` (plus the `demo` Spring profile) brings up the whole demo scenario together, instead of running the simulator as a separate manual `npm run dev`.
